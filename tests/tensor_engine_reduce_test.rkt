...
;;
;; Synthesize weight matrix to perform reduction using tensor engine
;;
;;;;
...
(define (spec x)
  ...
  (nks::matrix-vect (nks::mtx-reduce nks::var-add mtx #:axis 0 #:prec 8))
)
...
(define (sketch x)
  ...
  (define mtx (nks::matrix x rows_b cols_b))
  (nks::matrix-vect (nki.isa.nc_matmul sym-mtx mtx #:prec 8))
)
